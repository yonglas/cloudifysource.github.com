...
This section contains custom properties that are used to extend existing drivers.

|_.Attribute|_.Description|
|cleanGsFilesOnShutdown|It defines whether files/folders should be deleted when Cloudify stops a machine (occurs when Cloudify uninstalls an application or initiates teardown). Values: true or false|
